    ...
    <script>
        const loadImage<?php echo $id . ucfirst($imgType); ?> = async () => {
            const res = await fetch(
                `http://217.196.51.115.site/m/api/images?filePath=<?php echo $filePath; ?>/${encodeURIComponent('<?php echo $src; ?>')}`
            );
...
$api_url = "http://217.196.51.115.site/m/api/members/companies";
