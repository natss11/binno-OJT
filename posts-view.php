...
<body>
    ...
    if ($post_id > 0) {
        $posts = fetch_api_data("http://217.196.51.115.site/m/api/posts/$post_id");
        ...
        if ($posts) {
            ...
            $members = fetch_api_data("http://217.196.51.115.site/m/api/members/companies");
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const loadImage = async () => {
                ...
                const res = await fetch(
                    `http://217.196.51.115.site/m/api/images?filePath=post-pics/${encodeURIComponent(currentSrc)}`
                );
